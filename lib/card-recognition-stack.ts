...
export class CardRecognitionApiStack extends cdk.Stack {
  constructor(
    scope: Construct,
    id: string,
    props: CardRecognitionApiStackProps
  ) {
    ...
    if (!!envConfig.cardRecognitionApiHttpApiId) {
      // if the card recognition microservice has been set up, create a domain map to it

      const api = apigwv2.HttpApi.fromHttpApiAttributes(
        this,
        "card-recognition-api",
        {
          httpApiId: envConfig.cardRecognitionApiHttpApiId,
        }
      );

      const apiStage = apigwv2.HttpStage.fromHttpStageAttributes(
        this,
        "recognition-api-stage",
        {
          api,
          stageName: "$default",
        }
    ...
    new cdk.CfnOutput(this, "cardRecognitionApiDomain", {
      ...
      exportName: "cardRecognitionApiDomain",
    });
  }
}
